...
## Use With Measurement Workflows

Use `InstrumentAxis` and `Session.scan_instrument()` to scan an external
instrument around a QICK experiment. Bind the axis to the instrument's read and
write callbacks, specify its units and limits, and provide a resource ID.
The scan records setpoints, readbacks and child runs, and attempts to restore
the initial setting when the scan ends.

The separate [laboratory procedures](../../lab/procedures/) contain DC flux
and TWPA workflows. See the [architecture and API guide](../../docs/REFACTOR_GUIDE.md)
for the acquisition and instrument lifecycle.

...
def plot(
    ...
    if plot_circle:
        ...
        if corrected_signal is None:
            ...
            circle_ax.scatter(np.real(signal), np.imag(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)
            if fit is not None:
            ...
            if plot_not_corrected:
                ...
                circle_ax.scatter(np.real(signal), np.imag(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=0.15*alpha_fit)
            # circle_ax.plot(np.real(corrected_signal), np.imag(corrected_signal), ".C0")
            circle_ax.scatter(np.real(corrected_signal), np.imag(corrected_signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)
            if fit is not None:
                if plot_not_corrected:
                    circle_ax.plot(np.real(fit), np.imag(fit), "-C1", alpha=0.15, zorder=zorder)
                circle_ax.plot(np.real(corrected_fit), np.imag(corrected_fit), "-C1", zorder=zorder)
    ...
    mag_ax.scatter(freq_disp, dB(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit, label=params_label)
    if fit is not None:
    ...
    if corrected_signal is None:
        ...
        arg_ax.scatter(freq_disp, deg(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)# , label=params_label)
        if fit is not None:
            arg_ax.plot(freq_disp, deg(fit), "-C1", zorder=zorder)
    else:
        if plot_not_corrected:
            # arg_ax.plot(freq_disp, deg(signal), ".C0", alpha=0.15)
            arg_ax.scatter(freq_disp, deg(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=0.15*alpha_fit)
        # arg_ax.plot(freq_disp, deg(corrected_signal), ".C0", label=params_label)
        arg_ax.scatter(freq_disp, deg(corrected_signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)# , label=params_label)
        if fit is not None:
            if plot_not_corrected:

"""Local defaults and allowed ranges for qubit_spec_ef.
...
class QubitSpecEFParameters(Parameters):
    # Starting offset from the configured qubit EF frequency (MHz), not an absolute frequency.
    start: float = -20.0  # Default value.

    # Final offset from the configured qubit EF frequency (MHz), not an absolute frequency.

...
def best_result(runs, target):
    ...
    for row in runs:
        result = row["result"]
        if result.experiment != "drag_ge" or result.targets != (target,):
            raise ValueError("DRAG scan must measure one selected ge target")
        try:
...
def updates(result, target):
    ...
    return CalibrationUpdates(
        {
            "drag_alpha_ge": cfg["drag_alpha_ge"],
            "drag_delta_ge": cfg["drag_delta_ge"],
            "shape_ge": "drag",
            "pulse_type_ge": "arb",
        },

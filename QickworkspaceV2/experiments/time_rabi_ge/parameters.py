"""Local defaults and allowed ranges for time_rabi_ge.
...
class TimeRabiGEParameters(Parameters):
    # Starting qubit const-pulse duration (us).
    start: float = Field(
        default=0.02,  # Used when this parameter is omitted.
        gt=0,  # Strictly greater than 0 (exclusive minimum).
    )

    # Final qubit const-pulse duration (us).
    stop: float = Field(

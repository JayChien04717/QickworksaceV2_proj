"""Local defaults and allowed ranges for qubit_temperature.
...
class TemperatureParameters(Parameters):
    # Starting EF drive amplitude in normalized QICK gain units, not dBm.
    start: float = Field(
    ...
    # Final EF drive amplitude in normalized QICK gain units, not dBm.
    stop: float = Field(
    ...
    reset_wait_us: float = Field(
        default=500,  # Used when this parameter is omitted.
        gt=0,  # Strictly greater than 0 (exclusive minimum).
    )

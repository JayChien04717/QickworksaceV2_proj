...
def detect_resonators(
    ...
    *,
    ...
    if prominence_sigma < 0 or width_penalty < 0 or phase_snap_hz < 0:
        raise ValueError("prominence_sigma, width_penalty, and phase_snap_hz must be non-negative.")
    ...
    dynamic_range = float(np.nanpercentile(magnitude, 95) - np.nanpercentile(magnitude, 5))
    prominence_floor = max(dynamic_range * 0.01, np.finfo(float).eps)
    prominence = max(float(np.nanstd(smooth_dips) * prominence_sigma), prominence_floor)
    ...
    if use_phase_reference:
        phase_refs, corrected_phase, phase_derivative, electrical_delay_s = phase_reference_candidates(
            frequency,
            transmission,
            max_candidates=None,
    ...
    selected_array = np.asarray(sorted(selected[:count], key=lambda index: frequency[index]), dtype=int)
    display_phase_refs = []
    for selected_index in selected_array:
        nearby = phase_refs[np.abs(frequency[phase_refs] - frequency[selected_index]) <= 0.025e9]
        if nearby.size:
            display_phase_refs.append(int(nearby[np.argmin(np.abs(nearby - selected_index))]))
    ...
    **detection_options
    ...
    for index in indices:
        ...
        if 0 < index < frequency.size - 1:
            ...
            quadratic, linear, _ = np.polyfit(local_frequency - origin, local_magnitude, 2)
            if quadratic > 0:
...
__all__ = ["detect_resonators", "fit_n_resonators", "phase_reference_candidates"]

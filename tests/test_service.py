...
def test_worker_result_has_real_arrays_and_image(accepted_record):
    payload = to_worker_result(accepted_record)
...
def test_worker_result_shots_preserve_named_axes_after_roundtrip(dims, shape, coords, tmp_path):
    import numpy as np
    ...
    payload = json.loads(json.dumps(to_worker_result(loaded), allow_nan=False))
...
def test_worker_result_reports_omitted_shots_and_honors_export_budget():
    import numpy as np
    from QickworkspaceV2 import ExperimentData, TraceData
    ...
    payload = to_worker_result(result, max_array_values=6)
